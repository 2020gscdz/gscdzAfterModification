...
public class UnsolProDB extends DB{
	...
	public ArrayList<Problem> list(){
		...
		try {
			...
			while(res.next()) {
				...
				problem.setCreateTime(res.getTimestamp("createTime"));
	...
	public Problem findById(int id) {
		...
		try {
			res=stmt.executeQuery(sql);
			while(res.next()) {
				problem =new Problem(id,res.getString("subject"),res.getString("solution"),res.getString("knowledgePoints"),res.getString("analysis"),res.getInt("state"),res.getTimestamp("createTime"),res.getInt("creUserId"),res.getString("answUserId"));
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
